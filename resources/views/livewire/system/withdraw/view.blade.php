<div x-init="$wire.getDeta" x-loading.disabled>
    ...
    <x-dashboard.container >
        <div style="display: grid; grid-template-columns:repeat(auto-fit, 170px); grid-gap:10px; justify-content:start; align-items:start">
            ...
            <div class="p-3 w-full bg-white rounded-lg shadow-sm">
                ...
                <x-hr/>
                <div class="text-5xl font-bold my-3">
                    {{$withdraw?->payable_amount ?? 0}} <span class="text-sm"> {{$withdraw?->user?->currency_sing}} </span>
                </div>
                Range
                <x-hr/>
                <div class=" text-xl font-bold">
            ...
            <x-dashboard.section.header>
                ...
                <x-slot name="content">
                    <span class="text-white">
                        {{$withdraw?->reject_for ?? 'Unknown reason'}}
                    </span>
                </x-slot>
            </x-dashboard.section.header>
        </div>
        ...
        <x-dashboard.section @class(['bg-indigo-900 text-white','hidden' => !$withdraw?->status])>
            <x-dashboard.section.header>
                ...
                <x-slot name="content">
                Confirm by  {{$withdraw?->confirm_by ?? 'N/A'}} at {{\Carbon\Carbon::parse($withdraw?->updated_at)->format('m:s')}}
                </x-slot>
            </x-dashboard.section.header>
        </x-dashboard.section>

        <x-dashboard.section x-loading.disabled @class(["mt-2", 'hidden' => $withdraw?->status])>
            <x-dashboard.section.header>
                <x-slot name="title">
                    ...
                    <div class="p-2 rounded-lg mt-2 border">
                        <form wire:submit.prevent="confirmPayment">
                            ...
                            <div class="md:flex justify-between items-center py-1 my-1">
                                ...
                                <x-text-input wire:model.lazy="paid_from" class="" placeholder="From where the payment has been done" />
                            </div>
                            <div class="text-xs">
                            ...
                            <div class="md:flex justify-between items-center py-1 my-1">
                                ...
                                <x-text-input wire:model.lazy="trx" class="" placeholder="Who make this payment" />
                            </div>
                            @error('trx')
                                <div class="text-red-900"> 
                                    {{$message}}
        ...
        <x-dashboard.section @class(['hidden' => $withdraw?->status || $withdraw?->is_rejected ])>
            <x-dashboard.section.header>
                ...
                <x-slot name="content">
                    Have an unprocessable payment? Wish to reject the payment requeest. Payment rejection process done by following procedures.
                </x-slot>
            </x-dashboard.section.header>
            <x-hr/>
            <form wire:submit.prevent="rejectPayment">
                ...
                <textarea wire:model="rMessage" id="" class="w-full rounded-lg" placeholder="Write Your Rejection Message .......... " rows="4"></textarea>
                @error('rMessage')
                    <div class="text-red-900"> 
                        {{$message}}
                    </d>
                @enderror
                <x-danger-button>Confirm Reject</x-danger-button>
