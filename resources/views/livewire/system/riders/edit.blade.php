                    ...
                    <div>
                        <form wire:submit.prevent='updateStatus'>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div >
                                    <p class="text-sm">Current Status is : <strong> {{$rider->status}} </strong>. Change status to - </p>
                                    <select id="" wire:model="requestStatus" class="rounded-lg py-1" >
                                        <option value="Select Status">-- Select -- </option>
                                        <option @if($rider->status == 'Active') selected @endif value="Active">Active</option>
                                        <option @if($rider->status == 'Pending') selected @endif value="Pending">Pending</option>
                                        <option @if($rider->status == 'Disabled') selected @endif value="Disabled">Disabled</option>
                                        <option @if($rider->status == 'Suspended') selected @endif value="Suspended">Suspended</option>
                                    </select>
