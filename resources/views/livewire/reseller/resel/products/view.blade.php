                    ...
                    <div class="flex"> 
                        ...
                        @if (!$ableToAdd)
                            <div class="p-2 bg-red-200 text-red-800">
                                You have reached the maximum number of products you can upload {{auth()->user()->resellerShop()->max_resell_product}}. Please delete some products to add new ones or <div class="text-blue-600">upgrade your plan</div>.
                            </div>
                        @else
                            {{-- <div class="p-2 text-xs bg-green-200 text-green-800">
                                You can add up to {{auth()->user()->resellerShop()->max_resell_product}} products. You have {{$totalReselProducts}} Reselling products.
                            </div> --}}
                ...
                <div class="lg:flex justify-between item-start p-2">
                    ...
                    <div class="w-full lg:w-1/2 py-3 lg:py-0 px-4 lg:px-0">
                        ...
                        <div class="text-2xl flex bold">
                            ...
                            @else 
                                <div style="font-weight:bold;font-size:22px; color:var(--brand-primary); margin-right:12px"> Price : {{$products->price}} TK </div>
                            @endif 
                        </div>
                                ...
                                <div>
                                    ...
                                    <div>
                                        ...
                                        <div class="text-lg text-indigo-900"> 
                                            ...
                                            <br>
                                            <x-nav-link href="{{route('shops', ['slug' => $owner->isVendor()->slug, 'id' => $owner->isVendor()->id])}}"  > visit shops <i class="fas fa-angle-right pl-2"></i> </x-nav-link>
                                        </div>
                                    </div>
                                    <x-hr/>
                                    <div>
        ...
        <div class="p-2 px-4">
            ...
            <div>
                <div class="text-sm">
                    ...
                    <ul list-item="number">
                        ...
                        <li>
                            <i class="fas fa-check-circle w-6 pr-2"></i> Product owner get a message from you that you are reselling this products. 
                        </li>
            ...
            <div class=" ">
                <div class="mb-2">
                    ...
                    <div class="mt-2 border rounded p-2 shadow">
                        <div class="flex justify-between mb-2">
                            ...
                            <input type="checkbox" name="isReselWithDiscountPrice" id="isReselWithDiscountPrice" value="true" wire:model.live="isReselWithDiscountPrice" style="width: 20px; height:20px;" />
                        </div>
                        {{-- @php
                        ...
                        @if ($isReselWithDiscountPrice)
                            <div>
                                <x-text-input placeholder="Discount Price" min="{{$products->price}}" type="number" wire:model.live="reselDiscountPrice" class="w-full" />
                            </div>
                        @endif
                    </div>
                </div>
                <div>
                    Profit :
                    @php
                        $mainPrice = $products->offer_type ? $products->discount : $products->price;
                    @endphp
                    @if ($isReselWithDiscountPrice)
                        <span class="text-red-500 px-2"> (with discount) </span>
                        <span class="text-red-500"> {{$reselDiscountPrice - $mainPrice}} </span>
                    @else
                        <span class="text-red-500 px-2"> (without discount) </span>
                        <span class="text-red-500"> {{$reselPrice - $mainPrice}} </span>
                    @endif
                </div>
                <x-hr/>
                <div>
            ...
            <x-hr/>
            For procced, click to confirm button. After successfully cloned, You can update resel product from your product list.
            <div class="flex justify-end items-start p-2">
                <x-primary-button type="button" wire:click="confirmClone"> <i class="fas fa-sync pr-2"></i> Confirm</x-primary-button>
            </div>
        </div>
    </x-modal>
    {{-- resel confirm modal  --}}
