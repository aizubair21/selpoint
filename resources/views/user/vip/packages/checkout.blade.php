...
<div class="" style="">
    {{-- <div style="display: grid; grid-template-columns:repeat(auto-fit, 225px); grid-gap:20px; justify-content:center" class="border p-2">
    ...
    {{-- if the user owner  --}}
    <div @class(["d-none d-flex border-bottom py-2 justify-content-between align-items-center input-group", 'd-block' => $ownerPackage??""])>
        <div>
            Package
        </div>
    </div>
        ...
        <div class="col-lg-7 w-100 px-3">
            <div class="bg-white">        
                <div class="text-left">
                    ...
                    <p>Please send TK {{$package->price ?? "0"}} to bellow number. And collect your Tansactions ID for further proccess. We need your Transactions ID to identify it's you.</p>
                            ...
                            @foreach ($package->payOption as $item)
                                ...
                                <div class="p-2 rounded border mb-1">
                                    ...
                                    <div class="input-group w-100">
                                        ...
                                        <button class="btn btn-sm ml-5 py-0" id="" onclick="copyPaymentNumber(this, 'paymentTo_{{$item->id}}')" >Copy</button>
                                    </div>
                                </div>
                            @endforeach
                        @endif
  ...
  <div class="modal fade" id="packageDetailsModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div @class(['row m-0 d-none' => $ownerPackage, 'block'])>
        ...
        <div @class(["py-4 px-2 col-12", 'col-lg-8' => $ownerPackage]) style="display: grid; grid-template-columns:repeat(auto-fit, 155px); grid-gap:20px; justify-content:center" >
    ...
    <div class="text-center my-3">
        <a href="{{route("user.package.confirm", ['id' => $id])}}" @class(["btn btn-lg bg_primary shadow text-white", 'd-none' => $ownerPackage??""])>Procces to Purchase <i class="fas fa-arrow-right mx-2"></i> </a>
